...
```go
// تابع InitDB بر اساس نوع دیتابیس، اتصال مناسب را ایجاد می‌کند
func InitDB(dbPath string) error {
    if config.IsMySQL() {
        // اتصال به MySQL با DSN ساخته شده از متغیرهای محیطی
...
این بخش یک سرور HTTP/HTTPS جداگانه اجرا می‌کند که:
- لینک ساب‌اسکریپشن (Subscription Link) برای کلاینت‌ها ارائه می‌دهد
...
### ۱. تعریف اینباند
از طریق پنل وب یک اینباند جدید تعریف می‌کنید:
...
### ۳. اتصال کلاینت‌ها
دو روش:
- **لینک مستقیم**: کپی لینک و افزودن به کلاینت (مثل v2rayNG, Streisand, ...)
